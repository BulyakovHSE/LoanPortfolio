...
namespace LoanPortfolio.WebApplication.Models
{
    public class LoanPaymentNotification : Notification
    {
        ...
        public override string Text =>
            $"<h2>Напоминание об оплате кредита в {_loan.CreditInstitutionName} {_loan.DatePayment}</h2>" +
            $"<p>Уважаемый(-ая) {User.FirstName} {User.LastName}!</p>" +
            $"<p>Напоминаем вам о том, что {_loan.DatePayment} необходимо оплатить кредит на сумму {_loan.Sum} ₽ в {_loan.CreditInstitutionName}</p>" +
        ...
        {
            ExecuteDateTime = executeDateTime == default(DateTime) ? _loan.DatePayment.AddDays(-1).SetTime(10, 00) : executeDateTime;
            _loan = expense;
